...
4. **App suite** — Browser, Camera (vision), Notes, Mail (Gmail OAuth), Calendar (Google Calendar), Weather (Open-Meteo + Nemotron), Maps (Google Places/Routes + MapKit + Nemotron).
...
For live Maps, enable **Places API (New)** and **Routes API** on the Google Cloud key. Without a Maps key, the app still opens with demo places.
